...
void radeontop_init_radeon(radeontop_context *context, int fd, int drm_major, int drm_minor) {
	...
	if (DRM_ATLEAST_VERSION(2, 42)) {
		if (!(ret = getgrbm_radeon(&out32)))
			context->getgrbm = getgrbm_radeon;
		else
		...
		context->mclk_max = 0;	// no max memory clock info on radeon

		if (!(ret = getsclk_radeon(&out32)))
			context->getsclk = getsclk_radeon;
		else
			drmError(ret, _("Failed to get shader clock"));

		if (!(ret = getmclk_radeon(&out32)))
			context->getmclk = getmclk_radeon;
		else
	...
	if (DRM_ATLEAST_VERSION(2, 39)) {
		...
		context->vramsize = gem.vram_size;
		context->gttsize = gem.gart_size;

		if (!(ret = getvram_radeon(&out64)))
			context->getvram = getvram_radeon;
		else
			drmError(ret, _("Failed to get VRAM usage"));

		if (!(ret = getgtt_radeon(&out64)))
			context->getgtt = getgtt_radeon;
		else
